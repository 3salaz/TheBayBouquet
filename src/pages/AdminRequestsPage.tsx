...
export default function AdminRequestsPage() {
  ...
  const newRequests = filteredRequests.filter((request) => request.status === "new")

  const confirmedRequests = filteredRequests.filter(
    (request) => request.status === "confirmed"
  )

  const readyRequests = filteredRequests.filter(
    ...
    <section className="space-y-8">
      ...
      {loading ? (
        ...
        <>
          ...
          {filter === "all" && readyRequests.length > 0 && (
            <div className="space-y-4">
              ...
              <div className="grid gap-4">
                {readyRequests.map((request) => (
                  <div
                    key={`ready-${request.id}`}
                    className="rounded-2xl border border-purple-200 bg-purple-50 p-6 shadow-sm"
                  >
                    ...
                    <button
                      ...
                      disabled={updatingId === request.id}
                      className="mt-4 inline-flex items-center justify-center rounded-xl bg-purple-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-purple-700 disabled:opacity-60"
                    >
                      {updatingId === request.id ? "Updating..." : "Mark Completed"}
                    </button>
                  </div>
                ))}
              </div>
